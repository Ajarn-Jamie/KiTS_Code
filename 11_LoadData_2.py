...
for i in range(1,210):
    ...
    im_dir = r'D:\DATA_KITS2019\plots_15\case_00%.3i' % i
    if not os.path.exists(im_dir):
        os.makedirs(im_dir)
        os.makedirs(im_dir+'\\vol')
        os.makedirs(im_dir+'\\seg')
    ...
    for j in range(0,x_prep.shape[0]):
        ...
        imfilename = 'case_00%.3i_vol_%.3i.png' % (i,j)
        impath = os.path.join(im_dir+'\\vol', imfilename)
        plt.savefig(impath,dpi=200)
        plt.clf()

        
##        segfilename = 'case_00%.3i_seg_%.3i.png' % (i,j)
##        segpath = os.path.join(im_dir+'\\seg', segfilename)
